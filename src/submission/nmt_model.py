...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        #embed_size = len(vocab.src) hidden_size = 512
        self.encoder = nn.LSTM(embed_size, hidden_size, bidirectional=True)
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size)
        #o = i * w  + b, 2*h = 2*hidden_size 
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        dec_init_state = (self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim=1)), self.c_projection(torch.cat((last_cell[0], last_cell[1]), dim=1)))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #Ybar_t = Ybar_t.squeeze()
        dec_state = self.decoder(Ybar_t, dec_state)
        #dec_state[0] = dec_hidden #dec_state[1] = dec_cell
        ...
        #O_t = torch.dropout(nn.functional.tanh(V_t), p=self.dropout)
